...
function initLanguage() {
    ...
    if (langType) {
        langType.addEventListener('click', toggleLanguage);
    }
    setLanguage(AppState.currentLang);
}

function toggleLanguage() {
    const newLang = AppState.currentLang === 'ENG' ? 'MAL' : 'ENG';
    setLanguage(newLang);
...
function setLanguage(lang) {
    ...
    if (lang === 'MAL') {
        html.setAttribute('lang', 'MAL');
        html.setAttribute('dir', 'rtl');
        body.setAttribute('data-lang', 'MAL');
        body.setAttribute('data-dir', 'rtl');
    } else {
        html.setAttribute('lang', 'ENG');
        html.setAttribute('dir', 'ltr');
        body.setAttribute('data-lang', 'ENG');
...
function updateLanguageUI() {
    const textElements = document.querySelectorAll('[data-text-ENG], [data-text-MAL]');
    textElements.forEach(element => {
        const engText = element.getAttribute('data-text-ENG');
        const malText = element.getAttribute('data-text-MAL');
        if (AppState.currentLang === 'MAL' && malText) {
            element.textContent = malText;
        } else if (AppState.currentLang === 'ENG' && engText) {
            element.textContent = engText;
        }
    });

    const placeholderElements = document.querySelectorAll('[data-placeholder-ENG], [data-placeholder-MAL]');
    placeholderElements.forEach(element => {
        const engPlaceholder = element.getAttribute('data-placeholder-ENG');
        const malPlaceholder = element.getAttribute('data-placeholder-MAL');
        if (AppState.currentLang === 'MAL' && malPlaceholder) {
            element.setAttribute('placeholder', malPlaceholder);
        } else if (AppState.currentLang === 'ENG' && engPlaceholder) {
            element.setAttribute('placeholder', engPlaceholder);
        }
    });
    ...
    if (langToggle) {
        ...
        if (langText) {
            langText.textContent = AppState.currentLang === 'ENG' ? 'MAL' : 'ENG';
        }
    }
}
...
function handleFormSubmit(e) {
    ...
    const message = AppState.currentLang === 'MAL' 
